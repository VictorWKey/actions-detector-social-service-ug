...
def plot_skeleton_animation(csv_file):
    ...
    x_min, x_max = df.iloc[:, 0::3].min().min(), df.iloc[:, 0::3].max().max()
    y_min, y_max = df.iloc[:, 1::3].min().min(), df.iloc[:, 1::3].max().max()
    z_min, z_max = df.iloc[:, 2::3].min().min(), df.iloc[:, 2::3].max().max()
    ...
    def update(frame):
        ...
        # Convertir la fila actual en coordenadas (x, y, z) de los joints
        joints = []
        for i in range(0, 99, 3):
            x, y, z = df.iloc[frame, i], df.iloc[frame, i+1], df.iloc[frame, i+2]
            joints.append((x, y, z))
...
csv_file = r"cleanData/martillar_Clean/martillar_14.csv" 
