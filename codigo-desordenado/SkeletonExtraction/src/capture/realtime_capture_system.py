"""
Código utilizado para realizar las capturas de cada una
de las actividades así como su registro en archivos csv por frame
"""
...
def showCamera(i, pipeline):
  ...
  while running:
    ...
    # Handle keyboard input (only for camera 0 to avoid conflicts)
    if i == 0:
      key = cv2.waitKey(1)
      if key == 32:  # Space key
        if capture:
          capture = False  # Stop current capture
        else:
          capture = True   # Start new capture
...
def updateWindow():
  ...
  if capture:
    cv2.putText(arrange_imgs, "CAPTURANDO... ESPACIO: Detener | ESC: Salir", (30, arrange_imgs.shape[0] - 30),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2, cv2.LINE_AA)
  else:
    cv2.putText(arrange_imgs, "ESPACIO: Iniciar captura | ESC: Salir", (30, arrange_imgs.shape[0] - 30),
